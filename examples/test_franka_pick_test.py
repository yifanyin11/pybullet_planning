    ...
    for split in episode_data[obj_class].keys():
        ...
        for task_type in list(episode_data[obj_class][split].keys()) :
            ...
            for i in range(args.num_envs):
                ...
                # env.world.draw_frame_axes(sorted_grasps[0].pose.translation, sorted_grasps[0].pose.rotation.as_matrix())
                grasps = [grasp.to_planning_grasp(block, robot, get_tool_link(robot)) for grasp in sorted_grasps]

                saved_world = WorldSaver()
                # path = plan(robot, current_joint_states, block, fixed=[floor], grasps=grasps, teleport=False)
                command = plan(robot, current_joint_states, block, fixed=[floor], grasps=grasps, teleport=False)


                saved_world.restore()

                # # Execute the motion command based on the selected display mode.
                # for joint_states in path:
                #     env.robot.set_joint_states(joint_states)
                #     env.world.step()
